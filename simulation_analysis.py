...
def plot_mut_spec(mut_dataframe, title: str, plot_prefix: str,
                  plot_dir: str,
                  y_lim=None, y_label='Frequencies',):
    """
    ...
    for i in range(96):
        plt.bar(mut_types_96[i], 
                mut_dataframe.iloc[i], 
                color=colors[color_groups[i]],
                )
    plt.xlabel('96 types')
    plt.ylabel(y_label)
    plt.title(title)
    plt. margins(x=0.005) # remove white spacing on left and right of plot areas
    ...
    labels = set(mut_types_6) # get unique item from list
    plt.legend(handles, labels, title='Groups')

    # savefig
    plt.savefig(os.path.join(plot_dir, f'{plot_prefix}.png'), dpi=150)
...
def plot_all_three(type_to_plot: str, filename: str, data: tuple, plot_dir: str,
    ...
    if type_to_plot == 'sum': # summing up all bins
        ...
        to_plot = [data_i.iloc[:, bin_range[0]:bin_range[1]] / num_bins for data_i in data]
...
if __name__ == "__main__":
    ...
    sim_exposure = pd.read_csv(f'simulated_data/{sample_name}.exposure.csv', header=None)
    inferred_exposure = pd.read_csv(f'results_signature_trajectories/{sample_name}/{sample_name}/mixtures.csv')
    ...
    inferred_exposure_arr = np.c_[np.zeros(4), inferred_exposure.iloc[:,1:].to_numpy()]

    # matrix multiplication
    sim_data = cosmic_sigs_sample @ sim_exposure_arr
    inferred_data = cosmic_sigs_sample @ inferred_exposure_arr

    data_to_plot = sim_sample.iloc[:, 1:] / 100, pd.DataFrame(sim_data), pd.DataFrame(inferred_data)
    # divide sim_sample by 100 muts to normalize for each bin to sum to 1 matching the other two dataframes
