...
class RandomData(object):
    ...
    def _generate_e_i(self, distribution="random"):
        ...
        if distribution == "constant":
            ...
            i = True
            while i:
                cp = rd.sample(range(3, self._time_points - 3),
                               self._change_points)
            ...
            idx = 0
            while idx < len(cp)-1:
                constant = np.random.dirichlet(
                    np.ones(len(self._signatures)), size=1)
                exposure = constant.reshape(constant.shape[1], 1)
                exp = np.concatenate(
                    (exp, np.repeat(exposure, cp[idx+1]-cp[idx], axis=1)), axis=1)
                idx += 1
            exposure = exp[:, 1:]
        return exposure
    ...
    def _calculate_probability(self, signature_matrix, exposure):
        ...
        sig_names = signature_matrix[0, 1:]
        # Find the indices of the desired columns
        indices = [[str(sn) for sn in sig_names].index(col)
                        for col in self._signatures]
        selected_signatures = signature_matrix[:, 1:][:, indices][1:].astype(float)
...
if __name__ =="__main__":
    ...
    parser.add_argument(
        "--timepoints", help="number of time points", default=50)
    ...
    for sig_comb in signatures:
        ...
        mut_prob = random._calculate_probability(signature_matrix, exposure)
